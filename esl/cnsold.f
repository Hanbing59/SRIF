      ...
      SUBROUTINE CNSOLD(U,N,BX,NXMAX,NBX,IBZ,UBX,NP,EM,SGSTAR,IP,
     1                SGAIN,MAXV,XSTAR,F,G,H,V,UPDATU,UPDATX,IERR)
C
...
C        UDGEN  <ESL>
C        RNK1   <ESL>
C        SDOT1  <ESL-BLAS>  SDOT FROM BLAS, BUT RETURNS DP
C
C    COGNIZANT PERSONS:
C
...
C
     ...
     1          SGAIN(MAXV,*),XSTAR(N),SGSTAR(*),EM(*),BX(NXMAX,*)
      LOGICAL UPDATU,UPDATX
      DOUBLE PRECISION  SDOT1,SUM,TMP,TMPA,TMPB
C
...
C
           IF (IBX .EQ. 0) THEN
              XSTAR(L) = XSTAR(L) -
     1                    SDOT1(N,SGAIN(1,J),1,XSTAR,1) + SGAIN(N+1,J)
           ELSE
              TMP        = SDOT1(N,SGAIN(1,J),1,XSTAR,1) ! GAMMA
              TMPA       = TMP*EM(J)                     ! GAMMAM
...
230             CONTINUE
                H(L) = 1.0
                CALL UDGEN(U,N,H,ZERO,G,IZ+IBX,F,V,IERR)
C
